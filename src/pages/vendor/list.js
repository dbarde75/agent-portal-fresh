...
import VendorView from 'sections/apps/vendor/VendorView';
...
const CustomerListPage = () => {
    ...
    () => [
      ...
      },
      {
        ...
        Cell: ({ row }) => {
          const { values } = row;
          return (
            <Stack direction="row" spacing={1.5} alignItems="center" onClick={() => {
  ...
  const renderRowSubComponent = useCallback(({ row }) => <VendorView data={data[Number(row.id)]} />, [data]);
  ...
  return (
    <MainCard content={false}>
      ...
      <AlertVendorDelete title={customerDeleteId} open={open} handleClose={handleClose} />
      {/* add customer dialog */}
    </MainCard>
  );
};
