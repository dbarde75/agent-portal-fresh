...
import { dispatch, useSelector } from 'store';//useSelector
...
function ReactTable({ columns, data, renderRowSubComponent }) {
  ...
  //   history(`/products/add-new-product`);
  // };
  ...
  return (
    <>
      ...
      >
        <DialogTitle ><Typography fontWeight="bold" fontSize={20}>Add New Product</Typography></DialogTitle>
        <AddProduct onCancel={handleClose}/>
      </Dialog>
      ...
      <Stack spacing={3}>
        <Stack direction={{ sm: 'row', xs: 'column' }} justifyContent="space-between" spacing={1} alignItems="center" sx={{ p: 3, pb: 0 }}>
          ...
          <Stack direction={{ sm: 'row', xs: 'column' }} alignItems="center" spacing={1}>
            ...
            <Button variant="contained" startIcon={<Add />} onClick={handleAddProductOpen} size="large">
              Add Product
            </Button>
          </Stack>
        </Stack>
...
const ProductList = () => {
  ...
  //getting product data from store (product) by calling getProducts() action from product reducer
  useEffect(() => {
          ...
          return (
            <Stack direction="row" spacing={1.5} alignItems="center">
              <Avatar
                ...
                color="secondary"
                size="sm"
                src={productImage(`./thumbs/${!values.image ? 'prod-9.png' : values.image}`)}
              />
              <Stack spacing={0}>
